...
describe('DeviceFingerprint', () => {
    ...
    describe('create', () => {
        it('should create a new device fingerprint', () => {
            const fingerprint = DeviceFingerprint_1.DeviceFingerprint.create(baseProps);
        ...
        it('should generate IDs for each fingerprint', () => {
            const fp1 = DeviceFingerprint_1.DeviceFingerprint.create(baseProps);
            const fp2 = DeviceFingerprint_1.DeviceFingerprint.create(baseProps);
        ...
        it('should generate different hash for different properties', () => {
            const fp1 = DeviceFingerprint_1.DeviceFingerprint.create(baseProps);
            const fp2 = DeviceFingerprint_1.DeviceFingerprint.create({
                ...baseProps,
        ...
        it('should include entropy in hash calculation', () => {
            const fp1 = DeviceFingerprint_1.DeviceFingerprint.create({
                ...baseProps,
    ...
    describe('fromExisting', () => {
        it('should recreate fingerprint with existing ID', () => {
            ...
            const fingerprint = DeviceFingerprint_1.DeviceFingerprint.fromExisting(originalId, baseProps);
            expect(fingerprint.id).toBe(originalId);
            expect(fingerprint.hash).toBeDefined();
        });
        it('should compute same hash as create', () => {
            const created = DeviceFingerprint_1.DeviceFingerprint.create(baseProps);
            const existing = DeviceFingerprint_1.DeviceFingerprint.fromExisting('some-id', baseProps);
    ...
    describe('equals', () => {
        it('should return true for same ID and hash', () => {
            const fp = DeviceFingerprint_1.DeviceFingerprint.create(baseProps);
            const sameId = 'test-id';
            const fp1 = DeviceFingerprint_1.DeviceFingerprint.fromExisting(sameId, baseProps);
            const fp2 = DeviceFingerprint_1.DeviceFingerprint.fromExisting(sameId, baseProps);
            expect(fp1.equals(fp2)).toBe(true);
        });
        it('should return false for different IDs', () => {
            const fp1 = DeviceFingerprint_1.DeviceFingerprint.fromExisting('id-1', baseProps);
            const fp2 = DeviceFingerprint_1.DeviceFingerprint.fromExisting('id-2', baseProps);
            expect(fp1.equals(fp2)).toBe(false);
        });
        it('should return false for different hashes', () => {
            const fp1 = DeviceFingerprint_1.DeviceFingerprint.fromExisting('same-id', baseProps);
            const fp2 = DeviceFingerprint_1.DeviceFingerprint.fromExisting('same-id', {
                ...baseProps,
                userAgent: 'Different',
            });
            expect(fp1.equals(fp2)).toBe(false);
    ...
    describe('toJSON', () => {
        ...
        it('should include optional fields when present', () => {
            ...
            const fingerprint = DeviceFingerprint_1.DeviceFingerprint.create(propsWithEntropy);
            const json = fingerprint.toJSON();
    ...
    describe('hash computation', () => {
        it('should be deterministic', () => {
            const fp1 = DeviceFingerprint_1.DeviceFingerprint.create(baseProps);
            const fp2 = DeviceFingerprint_1.DeviceFingerprint.create(baseProps);
            const fp3 = DeviceFingerprint_1.DeviceFingerprint.create(baseProps);
            expect(fp1.hash).toBe(fp2.hash);
        ...
        it('should handle missing optional fields', () => {
            ...
            const fingerprint = DeviceFingerprint_1.DeviceFingerprint.create(minimalProps);
            expect(fingerprint.hash).toBeDefined();
            expect(fingerprint.hash).toMatch(/^[a-f0-9]{64}$/);
        });
    });
});
//# sourceMappingURL=DeviceFingerprint.spec.js.map